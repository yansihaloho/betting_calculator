...
function extractDepan2D(resultData: ResultRow[]): { num: string; tanggal: string; slot: string }[] {
  const out: { num: string; tanggal: string; slot: string }[] = [];
  for (const row of resultData) {
    for (const slot of [...TIME_SLOTS].reverse()) {
      const v = String(row[slot] || "");
      if (/^\d{4}$/.test(v)) {
        out.push({ num: v.slice(0, 2), tanggal: row.tanggal, slot });
      }
    }
  }
  return out;
...
export default function TwoDDepanPage({ resultData, isDark }: Props) {
  ...
  const analysis = useMemo(() => {
    ...
    const recent = allDepan.slice(0, 30).map(e => e.num);
    return { killed, live, liveWithStats, freq, recent };
  }, [allDepan, killCount, mode]);

  function doCopy(text: string, key: string) {
    if (copyText(text)) { setCopied(key); toast.success("Dicopy!"); setTimeout(() => setCopied(null), 2000); }
  }

  function buildWatermark(depan: typeof allDepan): string {
    if (!depan.length) return "";
    const e = depan[0];
    return `${e.tanggal}|${e.slot}|${e.num}`;
  }
  function findWatermarkIdx(depan: typeof allDepan, wm: string): number {
    return depan.findIndex(e => `${e.tanggal}|${e.slot}|${e.num}` === wm);
  }
  function computeHitsFromWatermark(entry: HistoryEntry, depan: typeof allDepan): { hits: string[]; newDrawsSeen: number } {
    if (!entry.watermark) return { hits: entry.hits, newDrawsSeen: 0 };
    const wmIdx = findWatermarkIdx(depan, entry.watermark);
    const newDraws = wmIdx > 0 ? depan.slice(0, wmIdx).map(e => e.num) : [];
    const hits = newDraws.filter(n => entry.live.includes(n));
    return { hits, newDrawsSeen: newDraws.length };
  }

  function saveToHistory() {
    const wm = buildWatermark(allDepan);
  ...
  function updateAccuracy(id: string) {
    const updated = history.map(entry => {
      ...
      const { hits, newDrawsSeen } = computeHitsFromWatermark(entry, allDepan);
    ...
    return (
      <div className={`flex items-center justify-center rounded-xl text-xs font-black aspect-square min-w-0
        ${status === "killed"
          ...
          ? isDark ? "bg-gradient-to-br from-blue-500/30 to-cyan-600/30 text-blue-400 ring-1 ring-blue-500/40" : "bg-blue-100 text-blue-700 ring-1 ring-blue-300"
  ...
  return (
    <div className="animate-slide-up space-y-4 pb-8">
      {/* Header */}
      <div className="rounded-[22px] bg-gradient-to-r from-blue-700 via-cyan-700 to-teal-600 text-white p-4 md:p-5 shadow-2xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-black">2D Depan</h1>
            <p className="opacity-70 text-xs mt-0.5">Kill {killCount} angka mati → {100 - killCount} angka hidup (2 digit pertama: AS+KOP)</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {[20, 25, 30, 35, 40].map(v => (
              <button key={v} onClick={() => setKillCount(v)}
                className={`px-2.5 py-1 rounded-xl text-xs font-black transition-all ${killCount === v ? "bg-white text-blue-700" : "bg-white/20 hover:bg-white/30"}`}>
                {v}
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2">
            ...
            {([["hybrid", "Hybrid"], ["recency", "Recency"], ["frequency", "Frekuensi"]] as const).map(([val, lbl]) => (
              <button key={val} onClick={() => setMode(val)}
                className={`px-2.5 py-1 rounded-xl text-xs font-black transition-all ${mode === val ? "bg-white text-blue-700" : "bg-white/20 hover:bg-white/30"}`}>
                {lbl}
              </button>
            ))}
          </div>
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          ...
          <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-white/15">📊 Data: {allDepan.length} draw</span>
      ...
      <div className={card}>
        ...
        {activeTab === "grid" ? (
          <div className="p-5 space-y-4">
            <div>
              <div className={`text-xs font-black mb-2 flex items-center gap-2 ${isDark ? "text-blue-400" : "text-blue-600"}`}>
          ...
          <div className="p-5">
            <div className={`text-xs font-black mb-3 ${isDark ? "text-blue-400" : "text-blue-600"}`}>TOP 20 ANGKA HIDUP (by frekuensi)</div>
              ...
              {analysis.liveWithStats.slice(0, 20).map((item, idx) => (
                <div key={item.num} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl ${sub}`}>
                  ...
                  <div className="flex-1 h-1.5 rounded-full bg-white/10">
                    <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-500" style={{ width: `${Math.min((item.freq / (analysis.liveWithStats[0]?.freq || 1)) * 100, 100)}%` }} />
                  </div>
      ...
      <div className={card}>
        <div className={`flex items-center gap-2 px-5 py-4 border-b ${isDark ? "border-white/8" : "border-slate-100"}`}>
          ...
          <span className="font-black text-sm">15 Draw Terakhir (Depan 2D)</span>
        </div>
        <div className="px-5 py-4 flex flex-wrap gap-2">
          {allDepan.slice(0, 15).map((e, i) => (
            <div key={i} className={`flex flex-col items-center gap-0.5`}>
              <span className={`px-2.5 py-1.5 rounded-xl text-sm font-black font-mono ${
                analysis.live.includes(e.num)
                  ? isDark ? "bg-blue-500/25 text-blue-300 ring-1 ring-blue-500/40" : "bg-blue-100 text-blue-700 ring-1 ring-blue-300"
      ...
      {showHistory && (
        <div className={card}>
          ...
          {history.length === 0 ? (
            <div className={`p-8 text-center text-sm ${muted}`}>Belum ada riwayat. Klik "Simpan Riwayat" untuk menyimpan analisa saat ini.</div>
          ) : (
                ...
                return (
                  <div key={entry.id} className="px-5 py-4 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        {hitRate !== null && (
                          <span className={`text-xs font-black ${hitColor}`}>{entry.hits.length} hit / {entry.newDrawsSeen} ({hitRate}%)</span>
                        )}
                        <button onClick={() => updateAccuracy(entry.id)} className={`p-1.5 rounded-lg ${isDark ? "hover:bg-white/10" : "hover:bg-slate-100"} transition-all`} title="Update akurasi">
                          <RefreshCw className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => deleteHistory(entry.id)} className={`p-1.5 rounded-lg ${isDark ? "hover:bg-red-500/20 text-red-400" : "hover:bg-red-50 text-red-400"} transition-all`}>
