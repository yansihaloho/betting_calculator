    ...
    fetchSohokartuResults(force)
      .then(rows => {
        if (rows && rows.length > 0) {
          const merged = applyManualOverrides(rows as typeof FALLBACK_RESULTS, manualOverrides);
          ...
          setResultData(applyManualOverrides([...FALLBACK_RESULTS], manualOverrides));
          setLastRefresh(new Date());
        ...
        {menu === "result" && (
          <div className="animate-slide-up space-y-4">
            ...
            <div className={`flex items-center gap-3 px-3 py-2 rounded-xl text-xs ${isDark ? "bg-white/5 border border-white/10" : "bg-slate-50 border border-slate-200"}`}>
              ...
              <span className={`flex items-center gap-1 ml-2 font-black ${isDark ? "text-white/40" : "text-slate-400"}`}>· Hit = jumlah slot menang hari itu</span>
            </div>
            ...
            {resultsByMonth.length === 0 ? (
              ...
              <div className="space-y-2">
                {resultsByMonth.map(({ month, rows }) => {
                  ...
                  const filtered = rows.filter(item => JSON.stringify(item).toLowerCase().includes(searchResult.toLowerCase()));
                  const toggleMonth = () => {
